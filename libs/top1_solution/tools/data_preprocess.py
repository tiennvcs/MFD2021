...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert to ICDAR format.')
    parser.add_argument('--input_dir', required=True,
        help='The path of directory, e.g, Tr00/')

    args = vars(parser.parse_args())
    root_path = args['input_dir']
    img_path = os.path.join(root_path, 'img')
    txt_path = os.path.join(root_path, 'gt')
    dst_path = os.path.join(root_path, 'gt_icdar')
    json_path = os.path.join(root_path, 'train_coco_sdk4.json')
    ...
    toCoco.save_coco_json(instance, json_path)
